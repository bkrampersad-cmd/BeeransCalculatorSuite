...
def calc_button(parent, text, command, clear_cmd=None, sample_cmd=None):
    ...
    if sample_cmd:
        ctk.CTkButton(row, text="Sample", command=sample_cmd,
                      fg_color="#EBF3FB", text_color=ACCENT,
                      hover_color="#D6E4F0",
...
class BasicCalcTab(ctk.CTkFrame):
    ...
    def _build(self):
        ctk.CTkLabel(self, text="Basic Calculator",
                     ...
                     text_color=NAV_BG).pack(anchor="w", padx=20, pady=(16, 12))
        ...
        for lbl, cmd in [("MC", self._mc), ("MS", self._ms),
                          ("M+", self._mplus), ("M−", self._mminus)]:
            ctk.CTkButton(_mrow, text=lbl, command=cmd,
                          width=54, height=28, fg_color="#EBF3FB",
                          hover_color="#B8CCE4", text_color=NAV_BG,
                          font=ctk.CTkFont("Segoe UI", 9, "bold"),
        ...
        legend_data = [
            ...
            ("MS",     "Overwrite the last-filled slot"),
            ("M+",     "Store in next slot  (cycles MR 1 → 2 → … → 8 → 1)"),
            ("M−",     "Subtract display from last-filled slot"),
            ("MR 1-8", "Left buttons — click any to recall that slot's value"),
        ]
        ...
        for btn_lbl, desc in legend_data:
            ...
            tk.Label(row2, text=btn_lbl, font=("Segoe UI", 9, "bold"),
                     fg=NAV_BG, bg=APP_BG, anchor="w", width=7).pack(side="left")
    ...
    def _mplus(self):
        """Store display value in next slot, then advance to the next."""
        try:
            val = float(self._current)
            s = self._next_slot
            self._mem_slots[s] = val
...
class LoanCalcTab(CalcTabMixin, ctk.CTkFrame):
    ...
    def _build(self):
        ctk.CTkLabel(self, text="Loan Calculators",
                     font=ctk.CTkFont("Segoe UI", 22, "bold"),
                     text_color=NAV_BG).pack(anchor="w", padx=20, pady=(16, 4))
    ...
    def _draw_diagram(self, count, wall, width):
        ...
        for i in range(count):
            ...
            c.create_rectangle(x0, TOP, x1, BOT,
                                fill="#D6E4F0", outline="#1565C0", width=3)
...
class BasicDepreciationTab(CalcTabMixin, ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(leg, text="📖  Method Guide",
                     ...
                     text_color=NAV_BG).pack(anchor="w", padx=14, pady=(8,2))
        legend_text = (
    ...
    def _build_sci(self, parent):
        ...
        for lbl, cmd in [("MC", self._sci_mc), ("M+", self._sci_mplus),
            ...
            ctk.CTkButton(sci_mctrl, text=lbl, command=cmd,
                          width=44, height=26, fg_color="#EBF3FB",
                          hover_color="#B8CCE4", text_color=NAV_BG,
                          font=ctk.CTkFont("Segoe UI", 9, "bold"),
...
class ConstructionCalcTab(CalcTabMixin, ctk.CTkFrame):
    ...
    def _calc_decking(self):
        try:
            ...
            self._dk_diag = dict(dl=dl, dw=dw, bwid=bwid, gap=gap, joist=joist, blen=blen)
    ...
    def _draw_area_diagram(self):
        ...
        F="#D6E4F0"; O="#1565C0"; DIM="#1F3864"; PAD=50
    ...
    def _draw_bt_diagram(self):
        ...
        for i in range(n):
            ...
            c.create_rectangle(x0,TOP,x1,BOT,fill="#D6E4F0",outline="#1565C0",width=2)
    ...
    def _draw_cn_diagram(self):
        ...
        if not d: self._dph(c,W,H); return
        shape=d["shape"]; MX=W//2; MY=H//2; F="#D6E4F0"; O="#1565C0"; DIM="#1F3864"; OFF=14
        ...
        if shape == "Slab":
            ...
            c.create_polygon([x0,y0+OFF,x0+OFF,y0,x0+pw+OFF,y0,x0+pw,y0+OFF],fill="#EBF3FB",outline=O,width=2)
    ...
    def _draw_cb_diagram(self):
        ...
        OL="#1565C0"; DIM="#1F3864"; OL_CB="#1F3864"
    ...
    def _draw_dk_diagram(self):
        ...
        F="#D6E4F0"; O="#1565C0"; DIM="#1F3864"; GRN="#276749"
        ...
        # ── Header line 1: board info (left)  |  suggested gap (right) ──
        ...
        # ── Header line 2: prevents stripping note (right) ────────────
        ...
        # ── Board rows: alternating half-board stagger ────────────
        y=deck_y0; row=0
    ...
    def _draw_lb_diagram(self):
        ...
        if not d: self._dph(c,W,H); return
        t=d["t"]; w=d["w"]; l=d["l"]
        F="#D6E4F0"; O="#1565C0"; DIM="#1F3864"; OFF=16; PAD=50
        ...
        c.create_polygon([x0,y0+OFF,x0+OFF,y0,x0+bw+OFF,y0,x0+bw,y0+OFF],fill="#EBF3FB",outline=O,width=2)
    ...
    def _draw_rf_diagram(self):
        ...
        F="#D6E4F0"; O="#1565C0"; DIM="#1F3864"; PAD=60
    ...
    def _draw_st_diagram(self):
        ...
        MX=W//2; MY=H//2
        F="#D6E4F0"; O="#1565C0"; DIM="#1F3864"; ST="#E8A020"; NF="#2F5496"
        ...
        # Panel border lines
        c.create_line(MX,0,MX,H,fill="#CBD5E0",width=1)
        c.create_line(0,MY,W,MY,fill="#CBD5E0",width=1)
    ...
    def _draw_vol_diagram(self):
        ...
        if not d: self._dph(c,W,H); return
        shape=d["shape"]; MX=W//2; MY=H//2; F="#D6E4F0"; O="#1565C0"; DIM="#1F3864"; OFF=16
        ...
        if shape == "Rectangular Box":
            ...
            c.create_polygon(pts2,fill="#EBF3FB",outline=O,width=1)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    def _build_parquet(self):
        ...
        self._pq_bl  =self._le(inner,'Board Length (in)','48')
    ...
    def _sample_parquet(self):
        ...
        self._pq_bl.set('48'); self._pq_bw.set('3.5'); self._pq_wst.set('10')
    ...
    def _draw_pq_diagram(self):
        ...
        PAD_L=52; PAD_R=90; TOP=22; BOT=78
        DW=W-PAD_L-PAD_R; DH=H-TOP-BOT
        ...
        self._va(c,x0-18,y0,y1_max,DIM,f'Width {rw/12:.1f} ft',right=False)
    ...
    def _build_cm_chart(self):
        ...
        for ri, (lbl, deg) in enumerate(corners):
            ...
            ctk.CTkLabel(row, text=lbl, width=100,
                         font=ctk.CTkFont("Segoe UI", 10, "bold"),
                         text_color=NAV_BG, anchor="center"
        ...
        ctk.CTkLabel(note, text="📐  How to use these angles on a compound miter saw:",
                     font=ctk.CTkFont("Segoe UI", 10, "bold"),
                     text_color=NAV_BG, anchor="w").pack(anchor="w", padx=10, pady=(6, 2))
        steps = (
...
class CompoundInterestCalcTab(CalcTabMixin, ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(info, text='📌  Rule of 72 — What Is It?',
                     ...
                     text_color=NAV_BG).pack(anchor='w', padx=14, pady=(10,4))
        ctk.CTkLabel(info,
...
class ITNetworkingCalcTab(CalcTabMixin, ctk.CTkFrame):
    ...
    def _build_subnet(self):
        ...
        ctk.CTkLabel(tab, text="Subnet / CIDR Calculator",
                     font=ctk.CTkFont("Segoe UI", 22, "bold"), text_color=NAV_BG
...
class BinaryHexCalcTab(CalcTabMixin, ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(r1,text='Number Conversion',font=ctk.CTkFont('Segoe UI',10,'bold'),
                     text_color=NAV_BG).pack(anchor='w',padx=12,pady=(6,2))
    ...
    def _build(self):
        ...
        def hdr(txt):
            ctk.CTkLabel(self, text=txt, font=ctk.CTkFont('Segoe UI',14,'bold'),
                         text_color=NAV_BG, anchor='w').pack(fill='x',padx=16,pady=(12,2))
...
class ConversionCalcTab(CalcTabMixin, ctk.CTkFrame):
    ...
    def _build_timedate(self):
        ...
        ctk.CTkLabel(s1, text="Date Difference",
                     font=ctk.CTkFont("Segoe UI",12,"bold"), text_color=NAV_BG
...
class MedicalCalcTab(CalcTabMixin, ctk.CTkFrame):
    ...
    def _build_bmi(self):
        ...
        ctk.CTkLabel(tab, text="BMI Calculator",
                     font=ctk.CTkFont("Segoe UI",24,"bold"), text_color=NAV_BG
                     ).pack(anchor="w", padx=20, pady=(16,4))
        ...
        ctk.CTkLabel(scale, text="BMI Reference Scale",
                     font=ctk.CTkFont("Segoe UI",16,"bold"), text_color=NAV_BG
    ...
    def _build_opioid(self):
        ...
        ctk.CTkLabel(ref, text="MME Conversion Factors",
                     font=ctk.CTkFont("Segoe UI",14,"bold"), text_color=NAV_BG
...
class ElectronicsCalcTab(CalcTabMixin, ctk.CTkFrame):
    ...
    def _build_ohms(self):
        ...
        ctk.CTkLabel(left, text="Enter any TWO known values — solve the others",
                     font=ctk.CTkFont("Segoe UI",11,"bold"), text_color=NAV_BG
        ...
        ctk.CTkButton(btn_row, text="Sample", command=self._sample_ohms,
                      fg_color="#EBF3FB", text_color=ACCENT, hover_color="#D6E4F0",
                      font=ctk.CTkFont("Segoe UI",10), width=72, height=36,
                      corner_radius=8).pack(side="left", padx=(0,6))
        ctk.CTkButton(btn_row, text="Clear", command=self._clear_ohms,
                      fg_color="#E2E8F0", text_color=TEXT, hover_color="#CBD5E0",
                      font=ctk.CTkFont("Segoe UI",10), width=72, height=36,
                      corner_radius=8).pack(side="left", padx=(0,6))
...
class App(ctk.CTk):
    ...
    def _build_sidebar(self):
        ...
        def _make_nav(title, back_cmd, items, select_cmd):
            ...
            btns = {}
            for name in items:
                b = ctk.CTkButton(scrl, text=name, anchor="w",
                                  fg_color="transparent", hover_color=NAV_HOV,
                                  text_color="white",
    ...
    def _build_about_frame(self):
        ...
        for col, (num, caption) in enumerate([
            ('48', 'Calculators'), ('8', 'Categories'), ('1', 'Beautiful Suite')
        ]):
        ...
        ctk.CTkLabel(q,
                     ...
                     font=ctk.CTkFont('Segoe UI', 13),
                     text_color=NAV_BG, justify='center',
    ...
    def _open_settings(self):
        ...
        s_theme = tk.StringVar(value=prefs.get("theme", "Light"))
        ctk.CTkOptionMenu(row, values=["Light", "Dark", "System"],
                          variable=s_theme,
                          fg_color=INPUT_BG, button_color=ACCENT,
                          text_color=TEXT, dropdown_text_color=TEXT,
                          font=ctk.CTkFont("Segoe UI", 13),
    ...
    _THEMES = {
        "Light": dict(APP_BG="#F0F4F8",CARD="#FFFFFF",INPUT_BG="#EBF3FB",
                      RES_BG="#D6E4F0",TEXT="#1A202C",MUTED="#718096",BORDER="#CBD5E0"),
        "Dark":  dict(APP_BG="#1A1E2E",CARD="#242840",INPUT_BG="#1E2A4A",
                      RES_BG="#1A2744",TEXT="#E2E8F0",MUTED="#94A3B8",BORDER="#2D3748"),
        "System":dict(APP_BG="#F0F4F8",CARD="#FFFFFF",INPUT_BG="#EBF3FB",
                      RES_BG="#D6E4F0",TEXT="#1A202C",MUTED="#718096",BORDER="#CBD5E0"),
    }

    def _on_close(self):
        # Hide the window immediately so close feels instant, then quit
        self.withdraw()
        self.quit()
    ...
    def _change_theme(self, choice):
        global APP_BG,CARD,INPUT_BG,RES_BG,TEXT,MUTED,BORDER
        ctk.set_appearance_mode(choice)
        p = self._THEMES.get(choice, self._THEMES["Light"])
        cmap = {APP_BG.lower():p["APP_BG"], CARD.lower():p["CARD"],
                INPUT_BG.lower():p["INPUT_BG"], RES_BG.lower():p["RES_BG"],
                TEXT.lower():p["TEXT"], MUTED.lower():p["MUTED"],
                BORDER.lower():p["BORDER"]}
        APP_BG=p["APP_BG"]; CARD=p["CARD"]; INPUT_BG=p["INPUT_BG"]
        RES_BG=p["RES_BG"]; TEXT=p["TEXT"]; MUTED=p["MUTED"]; BORDER=p["BORDER"]
        self.configure(fg_color=APP_BG)
        self.content.configure(fg_color=APP_BG)
        self._theme_walk(self, cmap)

    def _theme_walk(self, w, cmap):
        for child in w.winfo_children():
            try:
                for attr in ("fg_color","text_color","border_color"):
                    try:
                        v=child.cget(attr)
                        key=(v[0] if isinstance(v,(list,tuple)) else v).lower()
                        if key in cmap: child.configure(**{attr:cmap[key]})
                    except: pass
                try:
                    bg=child.cget("bg").lower()
                    if bg in cmap: child.configure(bg=cmap[bg])
                except: pass
                try:
                    fg=child.cget("fg").lower()
                    if fg in cmap: child.configure(fg=cmap[fg])
                except: pass
            except: pass
            self._theme_walk(child, cmap)
